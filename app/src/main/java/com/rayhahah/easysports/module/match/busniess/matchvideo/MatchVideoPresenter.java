...
public class MatchVideoPresenter extends RBasePresenter<MatchVideoContract.IMatchVideoView> implements MatchVideoContract.IMatchVideoPresenter {
    public MatchVideoPresenter(MatchVideoContract.IMatchVideoView view) {
        super(view);
    }

    @Override
    public void getVideoData(String mid) {
        addSubscription(MatchApiFactory.getMatchVideo(mid).subscribe(new Consumer<MatchVideo>() {
            @Override
            public void accept(@NonNull MatchVideo matchVideo) throws Exception {
                mView.getVideoDataSuccess(matchVideo.data);
            }
